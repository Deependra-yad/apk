    ...
    <div className="flex-1 flex flex-col h-full overflow-y-auto p-4 space-y-4 no-scrollbar">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-400">Recent voice and video calls</p>
        </div>

        <button
          onClick={() => setIsNewCallModalOpen(true)}
          className="p-2.5 rounded-xl bg-gradient-to-r from-liquid-accent to-liquid-secondary text-white font-semibold text-xs flex items-center gap-1.5 shadow-[0_0_15px_rgba(0,210,255,0.3)] hover:brightness-110 transition-all"
        >
          <Plus size={16} />
          <span>New Call</span>
        </button>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className={`text-sm font-semibold ${isMissed && !isOutgoing ? 'text-red-400' : 'text-white'}`}>
                ...
                <button
                  ...
                  className="p-2.5 rounded-full bg-white/10 hover:bg-liquid-accent/20 text-gray-300 hover:text-liquid-accent transition-all"
      ...
      {isNewCallModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-xl p-4">
          <div className="w-full max-w-md bg-liquid-base/95 border border-white/10 rounded-3xl p-6 shadow-2xl flex flex-col gap-4">
            <div className="flex justify-between items-center pb-2 border-b border-white/5">
              <h3 className="text-lg font-bold text-white">Start a New Call</h3>
            ...
            <div className="max-h-72 overflow-y-auto space-y-2 no-scrollbar">
              {users.map((u) => (
                <div key={u.id} className="flex items-center justify-between p-2.5 rounded-xl hover:bg-white/5">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400 truncate max-w-[160px]">{u.about || 'Liquid user'}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="p-2 rounded-full bg-white/10 hover:bg-green-500/20 text-gray-300 hover:text-green-400"
                      title="Voice Call"
                    >
                      <Phone size={16} />
                    </button>
                    <button
                      ...
                      className="p-2 rounded-full bg-white/10 hover:bg-liquid-accent/20 text-gray-300 hover:text-liquid-accent"
                      title="Video Call"
                    >
                      <Video size={16} />
                    </button>
