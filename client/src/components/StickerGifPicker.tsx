  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between pb-2 border-b border-white/10">
        <div className="flex gap-2">
          <button
            ...
            className={`px-3 py-1.5 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-all ${
              activeTab === 'stickers' ? 'bg-liquid-accent text-liquid-dark font-bold shadow-sm' : 'text-gray-400 hover:text-white'
            }`}
          >
            <Sparkles size={13} />
            <span>Stickers</span>
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-all ${
              activeTab === 'gifs' ? 'bg-liquid-accent text-liquid-dark font-bold shadow-sm' : 'text-gray-400 hover:text-white'
            }`}
          >
            <ImageIcon size={13} />
            <span>GIFs</span>
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-all ${
              activeTab === 'emojis' ? 'bg-liquid-accent text-liquid-dark font-bold shadow-sm' : 'text-gray-400 hover:text-white'
            }`}
          >
            <Smile size={13} />
            <span>Emojis</span>
        ...
        <button onClick={onClose} className="text-gray-400 hover:text-white text-xs p-1">
          ✕
        </button>
      </div>
        ...
        <div className="max-h-64 overflow-y-auto no-scrollbar space-y-3">
          {stickerPacks.map((pack, pIdx) => (
            <div key={pIdx} className="space-y-1.5">
              <span className="text-[11px] font-semibold text-gray-400 block px-1">{pack.name}</span>
              <div className="grid grid-cols-3 gap-2">
                {pack.stickers.map((st: any) => (
                  <motion.div
                    ...
                    className="aspect-square rounded-2xl bg-black/40 border border-white/10 p-2 flex flex-col items-center justify-center cursor-pointer hover:border-liquid-accent/50 group"
                  >
                    <img src={st.url} alt={st.name} className="w-14 h-14 object-cover rounded-xl" />
                    <span className="text-[10px] text-gray-400 truncate mt-1">{st.name}</span>
                  </motion.div>
                ))}
              </div>
            </div>
      ...
      {activeTab === 'gifs' && (
        <div className="space-y-2.5">
          <div className="h-9 bg-black/40 rounded-xl px-3 flex items-center gap-2 border border-white/10">
          ...
          <div className="max-h-60 overflow-y-auto no-scrollbar grid grid-cols-2 gap-2">
            {gifs.map((g) => (
              <motion.div
                ...
                className="rounded-xl overflow-hidden bg-black/40 border border-white/10 aspect-video cursor-pointer"
              >
      ...
      {activeTab === 'emojis' && (
        <div className="max-h-64 overflow-y-auto no-scrollbar space-y-2">
          {EMOJI_CATEGORIES.map((cat, cIdx) => (
            <div key={cIdx}>
              <span className="text-[10px] text-gray-400 font-semibold uppercase tracking-wider block mb-1">{cat.name}</span>
              <div className="grid grid-cols-6 sm:grid-cols-7 gap-1">
                {cat.emojis.map((em, eIdx) => (
                  <button
                    ...
                    className="p-1.5 text-xl hover:bg-white/10 rounded-xl transition-transform hover:scale-125 flex items-center justify-center"
                  >
                    {em}
                  </button>
                ))}
