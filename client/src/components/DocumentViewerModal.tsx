  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        exit={{ opacity: 0 }}
        onClick={onClose}
        className="fixed inset-0 z-50 bg-background/90 backdrop-blur-2xl flex items-center justify-center p-4 sm:p-6"
      >
        <motion.div
          ...
          className="w-full max-w-4xl h-[85vh] bg-liquid-base/95 border border-foreground/10 rounded-3xl overflow-hidden shadow-2xl flex flex-col justify-between"
        >
          {/* Header */}
          <div className="h-16 px-6 border-b border-foreground/10 flex items-center justify-between bg-background/40">
            <div className="flex items-center gap-3 overflow-hidden">
              ...
              <div className="overflow-hidden">
                ...
                <span className="text-[11px] text-foreground/60 font-mono">{fileSize || 'Document'}</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={onClose}
                className="p-2.5 text-foreground/60 hover:text-foreground rounded-xl hover:bg-foreground/10 transition-colors"
              >
                <X size={20} />
              </button>
            </div>
          ...
          <div className="flex-1 w-full h-full overflow-hidden bg-background/60 relative flex items-center justify-center">
            {isPdf ? (
              <iframe
              ...
              <iframe
                src={fileUrl}
                className="w-full h-full border-none p-6 text-foreground font-mono text-xs bg-background/80 overflow-auto"
                title="Code Preview"
              />
            ) : (
              <div className="flex flex-col items-center text-center p-8">
                ...
                <h4 className="text-lg font-bold text-foreground mb-2">{fileName}</h4>
